...
launch_latency_tests() {
  ...
  cqfd init
  cqfd run ansible-playbook -i ${INVENTORY_VM} -i ${INVENTORY_PUBLISHER} -i ${ANSIBLE_INVENTORY} playbooks/configure_latency_tests.yaml
  cqfd run ansible-playbook -i ${INVENTORY_VM} -i ${INVENTORY_PUBLISHER} -i ${ANSIBLE_INVENTORY} playbooks/run_latency_tests.yaml   -e "pcap=${PCAP}" \
