...
launch_latency_tests() {
  ...
  git clone -b ci-integration https://github.com/seapath/svtrace-ansible
  cd svtrace-ansible
  echo ${ANSIBLE_INVENTORY}
  git clone -b svtrace "${PRIVATE_INVENTORIES_REPO_URL}" inventories_private
  cqfd init
  cqfd run ansible-playbook \
  --limit "localhost,sv_publisher,guest0,yoctoCI-aaeon" \
  playbooks/configure_latency_tests.yaml \
  -i ${INVENTORY_VM} \
  -i ${INVENTORY_PUBLISHER} \
  -i inventories_private/ci_yocto_standalone_aaeon.yaml \
  ...
  cp ${WORK_DIR}/svtrace-ansible/tests_results/data/ci_latency_tests_yoctoCI-aaeon/results/latency_tests.adoc"${WORK_DIR}/ci/openlab/include/latency_tests_yoctoCI-aaeon.adoc"
  mv ${WORK_DIR}/svtrace-ansible/tests_results/data/results/histogram*.png ${WORK_DIR}/ci/openlab/doc/
